...
for(i in 1:length(averageTPM_removal10_ls)){
  ...
  p<-ggplot(data=averageTPM_oneSite_removal10, aes(y=log10(HS+1),x=log10(nonHS+1), label=locusTag))+theme_classic()+coord_fixed()+labs(title=paste0(site,"; cor=",round(cor(averageTPM_oneSite_removal10[,1:2])[1,2],2)), x="log10(TPM)\nnonHS", y="HS\nlog10(TPM)\n")+geom_point(alpha=0.4,size=1,shape=1)+scale_x_continuous(breaks=seq(0,6,1), limits=c(0,6)) + scale_y_continuous(breaks=seq(0,6,1), limits=c(0,6))+geom_abline(intercept = 0, slope = 1, linetype="dashed",color="grey")
  #+labs(title=paste0(site,": ",nrow(averageTPM_oneSite_removal10)," genes remaining"))+xlim(0,6)+ylim(0,6)#+geom_text(data=subset(averageTPM_oneSite_removal10,locusTag=="BF9343_RS18065"),angle = 0, size =2, alpha=0.5, color="red",hjust = 0, nudge_x = 0.05)+geom_point(alpha=0.5,shape=1)+geom_point(data=subset(averageTPM_oneSite_removal10,locusTag=="BF9343_RS18065"), color="red")
  #print(p)
  ggplots_ls[[i]] <- p
}
# Plot figure 1d lumen ---------------------------------------------------------------
print(ggplots_ls[[1]])
# Plot figure 1d mucus ---------------------------------------------------------------
print(ggplots_ls[[2]])
# Plot figure 1d tissue ---------------------------------------------------------------
print(ggplots_ls[[3]])
